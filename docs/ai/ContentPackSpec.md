...
- `places[].storyMode`（故事模式扩展，v2.1 新增）
  - `narrativeText` (string, 可选)：200-400字的沉浸式叙事文本，用于故事模式中呈现该地点的完整故事。应当写得引人入胜，让用户仿佛身临其境。
  - `hook` (string, 可选)：≤30字的悬念钩子/导读语，吸引用户打开此节点（如"阿宝在此赚到了第一桶金"）
  - `transitionTo` (string, 可选)：≤50字的过渡语，引导用户前往下一个节点（如"带着新的身份，他转身走向黄河路..."）
  - `chapterTitle` (string, 可选)：该节点在故事线中的章节标题（如"第三章·黄河路的黄金年代"）
  - `mood` (string, 可选)：该节点的情绪氛围标签，用于 UI 渲染（如 `tense`/`nostalgic`/`joyful`/`melancholic`/`dramatic`/`peaceful`/`mysterious`）
